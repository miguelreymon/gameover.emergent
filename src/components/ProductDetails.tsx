  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      <div ref={addToCartRef} className="space-y-3">
        ...
        {allOutOfStock && (() => {
          const phone = (siteContent as any)?.footer?.whatsAppNumber || '';
          if (!phone) return null;
          const message = encodeURIComponent(
            `Hola, ¿tenéis alguna ${product.name || 'consola'} en stock en reserva? Me interesa comprar una.`
          );
          return (
            <div
              className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 space-y-3"
              data-testid="out-of-stock-whatsapp-block"
            >
              <p className="text-sm text-slate-700 leading-relaxed">
                ¿Sin stock? <strong>Pregúntanos por WhatsApp</strong> si nos queda alguna consola
                en reserva: a veces tenemos unidades nuevas todavía sin publicar o estamos a punto
                de recibir reposición. Te lo confirmamos al momento.
              </p>
              <a
                href={`https://wa.me/${phone}?text=${message}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#1ebe5b] text-white font-semibold py-3 px-4 rounded-lg transition-colors"
                data-testid="out-of-stock-whatsapp-btn"
              >
                <svg
                  viewBox="0 0 24 24"
                  width="22"
